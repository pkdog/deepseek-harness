...
Before mounting profile rows, the `dsh` launcher computes one immutable package-resolution generation from the installation and ordered bundle dependency graphs. Runtime mode installs the generation through Node's ESM and CommonJS resolvers without creating fallback links; link mode materializes it instead. The launcher defaults to runtime mode, except for a TypeScript source launch (`node --import tsx/esm apps/cli/src/bin.ts`), which defaults to link mode: tsx projects workspace specifiers onto `src` while the runtime resolver re-resolves a routed package through its manifest `exports` onto the built `lib/`, so the two backends would load one package twice and split module identity across the copies. Plain Node callers of `runProfile` may select either mode explicitly, or dual mode to materialize and verify. Packaged executables and the Electron Host always use runtime mode.
